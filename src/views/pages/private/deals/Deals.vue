          ...
          <div class="flex ml-14 gap-10 text-sm uppercase w-full lg:w-[700px] text-gray-600 tracking-tight">
            ...
            <div class="flex items-center">
              ...
              <span class="ml-2 text-xl font-semibold text-gray-500 tracking-tight">
                {{ '$' + table.pagination.meta.estimatedSizeSum.toLocaleString('en-US', { maximumFractionDigits: 0 }) }}
              </span>
            </div>
            <div class="flex items-center">
      ...
      <template #default>            
          ...
          >
            ...
            <template #cell-next_task="{item}">                  
              <NextTaskField :task="item?.lastActiveTask" @submit="(args) => onTaskFormSubmit({record: item, ...args})" />
            </template>
      ...
      <template v-if="!table.loading" #footer>
        <div class="flex items-center">
          ...
          <div class="flex ml-14 gap-10 text-sm uppercase w-full lg:w-[550px] text-gray-500 tracking-tight">
            ...
            <div class="flex items-center">
              <span>{{ trans('deals.labels.quoted_value') }}</span>
              <span class="ml-2 text-xl font-semibold text-gray-400 tracking-tight">
...
function onTableFilter({column, value}) {
    if (column.key == 'owner' || column.key == 'source' || column.key == 'stage' || column.key == 'branch' || column.key == 'branch' || column.key == 'status' || column.key == 'creator' || column.key == 'category') {
...
function fetchSmartList(id) {
  smartListservice.find(id).then((res) => {
    ...
    if (smartList.resource_type != 'deal') {
...
function updateColumnsForSmartList() {
  ...
  if (categoryFilter.value) {  
    let selectedCategories = categoryFilter.value.split(',').map(item => {
      return dealCategories.find(option => option.id == item);
    });
...
function onSmartListSave({name}) {
  smartListservice.store({
    ...
    if (res.status == 200 || res.status == 201) {
      toast.success();
      router.push({name: 'deals.list', params: {id: res.data.data.id}})
    }
  });
}
...
onMounted(async () => {
  page.isLoading = true;
  if (!route.params.id) {
    page.title = trans('global.pages.publicity');
  ...
  smartLists = await smartListservice.index({'filter[resource_type]': 'deal'}).then(res => res.data.data);  
